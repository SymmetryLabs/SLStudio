...
public class UITreeLeaves extends UI3dComponent {
    ...
    public UITreeLeaves(P3LX lx, PApplet applet, OfficeTreeModel tree) {
        this.lx = lx;
        this.tree = tree;
    ...
    class LeafShape extends PShapeOpenGL {
        ...
        LeafShape(PGraphics pg) {
            ...
            setFill(false);
            beginShape(QUADS);
            for (OfficeTreeModel.Leaf leaf : tree.leaves) {
        ...
        void updateColors(PGraphics pg, int[] colors) {
            ...
            if (BIG_ENDIAN) {
                for (int i = 0; i < colors.length; i += OfficeTreeModel.Leaf.NUM_LEDS) {

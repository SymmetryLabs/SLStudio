...
public class UITreeStructure extends UI3dComponent {
    ...
    private final OfficeTreeModel tree;
    private static final int WOOD_FILL = 0xFF281403;

    public UITreeStructure(OfficeTreeModel tree) {
        this.tree = tree;
        addChild(new UICylinder(
            OfficeTreeModel.TRUNK_DIAMETER/2, OfficeTreeModel.TRUNK_DIAMETER/4,
            -OfficeTreeModel.LIMB_HEIGHT, 6*FEET, 8, WOOD_FILL));
        for (OfficeTreeModel.Limb limb : tree.limbs) {
            addChild(new UILimb(limb));
        }
        for (OfficeTreeModel.Branch branch : tree.branches) {
    ...
    public static class UILimb extends UI3dComponent {
        ...
        static {
            section1 = new UICylinder(OfficeTreeModel.Limb.SECTION_1.radius, OfficeTreeModel.Limb.SECTION_1.len, 5, WOOD_FILL);
            section2 = new UICylinder(OfficeTreeModel.Limb.SECTION_2.radius, OfficeTreeModel.Limb.SECTION_2.len, 5, WOOD_FILL);
            section3 = new UICylinder(OfficeTreeModel.Limb.SECTION_3.radius, OfficeTreeModel.Limb.SECTION_3.len, 5, WOOD_FILL);
            section4 = new UICylinder(OfficeTreeModel.Limb.SECTION_4.radius, OfficeTreeModel.Limb.SECTION_4.len, 5, WOOD_FILL);
        }

        private final float azimuth;
        private final float y;
        private final OfficeTreeModel.Limb.Size size;

        public UILimb(OfficeTreeModel.Limb limb) {
            this(limb.y, limb.azimuth, limb.size);
        }

        public UILimb(float azimuth, OfficeTreeModel.Limb.Size size) {
            this(0, azimuth, size);
        }
        ...
        public UILimb(float y, float azimuth) {
            this(y, azimuth, OfficeTreeModel.Limb.Size.FULL);
        }

        public UILimb(float y, float azimuth, OfficeTreeModel.Limb.Size size) {
            this.y = y;
        ...
        public void onDraw(UI ui, PGraphics pg) {
            ...
            if (this.size == OfficeTreeModel.Limb.Size.FULL) {
                section1.onDraw(ui, pg);
                pg.translate(0, section1.len, 0);
            }
            if (this.size != OfficeTreeModel.Limb.Size.SMALL) {
    ...
    public static class UIBranch extends UI3dComponent {
        ...
        private final OfficeTreeModel.Branch branch;
        ...
        public UIBranch(OfficeTreeModel.Branch branch) {
            this.branch = branch;
            for (OfficeTreeModel.LeafAssemblage assemblage : branch.assemblages) {
    ...
    public static class UILeafAssemblage extends UI3dComponent {
        ...
        private final OfficeTreeModel.LeafAssemblage assemblage;
        private static final UICylinder cylinder;

        static {
            cylinder = new UICylinder(.4f*INCHES, .4f*INCHES, -12f*INCHES, OfficeTreeModel.LeafAssemblage.LEAVES[8].y, 5, WOOD_FILL);
        }

        public UILeafAssemblage(OfficeTreeModel.LeafAssemblage assemblage) {

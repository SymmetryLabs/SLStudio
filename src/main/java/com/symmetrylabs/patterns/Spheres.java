    ...
    public void run(double deltaMs) {
        ...
        for (LXPoint p : model.points) {
            ...
            for (Sphere s : spheres) {
                float d = (float)Math.sqrt((float)Math.pow(p.x - s.x, 2) + (float)Math.pow(p.y - s.y, 2) + (float)Math.pow(p.z - s.z, 2));
                float r = (s.radius); // * (sinLfoValue + 0.5));
                value = Math.max(0, 1 - Math.max(0, d - r) / 10);

                c = LXColor.blend(c, lx.hsb(s.hue, 100, Math.min(1, value) * 100), LXColor.Blend.ADD);
            }

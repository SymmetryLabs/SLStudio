...
public class Noise extends DPat {
    ...
    @Override
    public void onActive() { zTime = MathUtils.random(500); zTheta=0; rtime = 0; ttime = 0; }
    ...
    void StartRun(double deltaMs) {
        ...
        zSin  = (float)Math.sin(zTheta);
        zCos  = (float)Math.cos(zTheta);
        ...
        if (pChoose.getValuei() != CurAnim) {
            ...
            case 5: N[0].set(0  ,400,100,2  ,15 ,3  ,90 );
                    ...
                    N[3].set(60 ,100,100,2  ,15 ,1.5f,270); pSharp.setValue(.5); break; // spark
            }
        }

        for (int i=0; i<_ND; i++) if (N[i].Active()) {
            N[i].sinAngle = (float)Math.sin(DEG_TO_RAD * N[i].angle);
            N[i].cosAngle = (float)Math.cos(DEG_TO_RAD * N[i].angle);
        }
    }
    ...
    int CalcPoint(LXVector p) {
        ...
        if (CurAnim == 6 || CurAnim == 7) {
            ...
            return lx.hsb(lxh(),100, 100 * (
                            MathUtils.constrain(1-50*(1-val(pDensity))*(float)Math.abs(p.y-(float)Math.sin(zTime*10  + p.x*(300))*.5 - .5),0,1) +
            (CurAnim == 7 ? MathUtils.constrain(1-50*(1-val(pDensity))*(float)Math.abs(p.x-(float)Math.sin(zTime*10  + p.y*(300))*.5 - .5),0,1) : 0))
            );
        }
        ...
        for (int i=0;i<_ND; i++) if (N[i].Active()) {
            ...
            float b = 1.8f * (iSymm==RadSym ? (zTime*n.speed+n.xoff-p.dist(mCtr)/n.xz)
                    : NoiseUtils.noise(p.x/n.xz+zx+n.xoff,p.y/n.yz+zy+n.yoff,p.z/n.zz+n.zoff));

            b +=  n.den/100 -.4 + val(pDensity) -1;
            c = LXColor.blend(c, lx.hsb(lxh() + n.hue, 100, c1c(b)), LXColor.Blend.ADD);
        }
        return c;
    }
}

...
public class NotationXLListener extends LXComponent implements LXMidiListener {
    ...
    int numSlots() {
        return 24; //return type == Type.ACTIVE ? 16 : 15;
    }

    NotationXLListener(LX lx, UI ui, LXMidiRemote midi, Type type) {
        super(lx);
        this.lx = lx;
        this.ui = ui;
        this.midi = midi;
        ...
        writeListeners = new LXParameterListener[24];
    ...
    public static void bindMidi(LX lx, UI ui) {
        ...
        ArrayList<LXMidiInput> inputs = new ArrayList<LXMidiInput>();
        ArrayList<LXMidiOutput> outputs = new ArrayList<LXMidiOutput>();


        for (LXMidiInput in : lx.engine.midi.inputs) {
            String name = in.getName();
            String desc = in.getDescription();
            if (name.contains("Notation")) {
                inputs.add(in);
            }
        }
        ...
        for (int i = 0; i < Math.min(2, inputs.size()); i++) {
            LXMidiInput in = inputs.get(i);
            LXMidiOutput out = outputs.get(i);
            in.open();
            out.open();
            LXMidiRemote notation = new LXMidiRemote(in, out);
            Type t = i == 0 ? Type.ACTIVE : Type.CUE;
            NotationXLListener listener = new NotationXLListener(lx, ui, notation, t);

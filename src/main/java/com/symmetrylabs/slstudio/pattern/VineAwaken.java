...
public class VineAwaken extends SLPattern<SLModel> {
    ...
    private final CompoundParameter speedParam =
        new CompoundParameter("speed", 12, 0, 1000);
    ...
    private final BooleanParameter resetParam =
        ...
        private final CompoundParameter minDist = new CompoundParameter("minDist", 100, 0, 1000);
        private final CompoundParameter maxDist = new CompoundParameter("maxDist", 400, 100, 1000);
    ...
    private void pickNewCenter() {
        ...
        c.loc = new LXVector(model.cx, model.cy, model.cz); //vectors.get(rand.nextInt(vectors.size()));
        c.dist = 0;
        c.killDist = 0;
    ...
    public void run(double elapsedMs) {
        ...
        for (Center c : centers) {
            float speed = Math.max(0, speedParam.getValuef());
            c.dist += dir * (speed / 1000f * (float) elapsedMs);
        ...
        for (Center c : centers) {
            if (c.dist >= maxDist.getValuef()) {
                dir = -1.0f;
            }
            if (c.dist <= minDist.getValuef()) {
                dir = 1.0f;
            }
            // c.dist = Math.max(0, c.dist);

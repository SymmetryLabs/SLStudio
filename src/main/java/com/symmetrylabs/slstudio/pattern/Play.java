...
public class Play extends DPat
{
    public class rAngle {
        ...
        float     fixAngle    (float a, float b) { return a<b ?
            (Math.abs(a-b) > Math.abs(a+2*PConstants.PI-b) ? a : a+2*PConstants.PI) :
            (Math.abs(a-b) > Math.abs(a-2*PConstants.PI-b) ? a : a-2*PConstants.PI)    ; }
...
//        addParameter(pShape);
    ...
    public class rWave {
        ...
        float val(float _x) {
            ...
            a  = a0*(float)Math.exp(-dist*damp) * (float)Math.exp(-Math.abs(_x-x0)/(.2*len)); // * max(0,1-t/dur)
...
//        if (row == 57) {KeyPressed = col; return; }
    ...
    public void StartRun(double deltaMs) {
        ...
        if (t<LastMeasure) {
            if (random(3) < 1) { curRandTempo = (int)MathUtils.random(4); if (curRandTempo == 3) curRandTempo = (int)MathUtils.random(4);    }
        ...
        switch (nTempo) {
            case 0:     t = t;                                break;
            case 1:     t = (float)((t*2. )%1.);                        break;
            case 2:     t = (float)((t*4. )%1.);                        break;
            case 3:     t = (float)((t*8. )%1.);                        break;
            case 4:     t = (float)((t*16.)%1.);                        break;
        }
        ...
        if ((t<LastBeat && pShape.getValuei()!=14) || KeyPressed>-1) {
            waves.add(new rWave(
                KeyPressed>-1 ? MathUtils.map(KeyPressed,0,7,0,1) : random(1),        // location
                bnc*10,            // bounciness
                7,                // velocity
                2*(1-amp)));    // dampiness
            KeyPressed=-1;
        ...
        switch (nTPat) {
            ...
            case 1:     t = PApplet.norm(MathUtils.sin(2*PConstants.PI*(t+PConstants.PI/2)),-1,1);        break;    // sin
            case 2:     t = t;                                     break;    // roll
            case 3:     t = MathUtils.constrain((int)((t*8)/7.),0,1);            break;    // quant
            case 4:     t = t*t*t;                                break;    // accel
            case 5:     t = MathUtils.sin(PConstants.PI*t*.5f);                        break;    // deccel
            case 6:     t = (float)(.5*(1-MathUtils.cos(PConstants.PI*t)));                    break;    // slide
        }
    ...
    public int CalcPoint(PVector Px) {
        ...
        switch (pShape.getValuei()) {
            ...
            case 1:        V.set(Pn.x, MathUtils.map(MathUtils.cos(PConstants.PI*t * Pn.x),-1,1,0,1)  , Pn.z);                             break;    // top tap
            case 2:        V.set(Pn.x, bnc*MathUtils.map(Pn.x<.5f?Pn.x:1-Pn.x,0,.5f ,0,t-.5f)+.5f, Pn.z);                break;    // V shape
            case 3:        V.set(Pn.x, Pn.x < cMidNorm.x ? MathUtils.map(Pn.x,0,cMidNorm.x, .5f,yt) :
                MathUtils.map(Pn.x,cMidNorm.x,1, yt,.5f), Pn.z);                  break;    //  Random V shape

            case 4:
                V.set(Pn.x,    .5f*(Pn.x < cMidNorm.x ?     MathUtils.map(Pn.x,0,cMidNorm.x, .5f,yt) : MathUtils.map(Pn.x,cMidNorm.x,1.0f, yt,.5f)) +  .5f*(Pn.z < cMidNorm.z ?     MathUtils.map(Pn.z,0,cMidNorm.z, .5f,yt) :
                    MathUtils.map(Pn.z,cMidNorm.z,1, yt,.5f)), Pn.z);         break;    //  Random Pyramid shape

            case 5:        V.set(Pn.x, bnc*MathUtils.map((Pn.x-.5f)*(Pn.x-.5f),0,.25f,0,t-.5f)+.5f, Pn.z);                break;    // wings
            case 6:        V.set(Pn.x, bnc*MathUtils.map((mp  -.5f)*(mp  -.5f),0,.25f,0,t-.5f)+.5f, Pn.z);                break;    // wings
            ...
            case 8:        r = amp*200 * MathUtils.map(bnc,0,1,1,MathUtils.sin(PConstants.PI*t));
                d = MathUtils.min(
            ...
            case 9:        r = amp*200 * MathUtils.map(bnc,0,1,1,MathUtils.sin(PConstants.PI*t));
                d = MathUtils.min(
                    distToSeg(Px.x, Px.y, a1.getX(r),a1.getY(r), a2.getX(r),a2.getY(r)),
                    MathUtils.min(distToSeg(Px.x, Px.y, a2.getX(r),a2.getY(r), a3.getX(r),a3.getY(r)),
            ...
            case 10:
                r = MathUtils.map(bnc,0,1,a1.r,amp*200*MathUtils.sin(PConstants.PI*t));
                return lx.hsb(lxh(),100,c1c(.9f+2*rad - MathUtils.dist(Px.x,Px.y,a1.getX(r),a1.getY(r))*.03f) );        // sphere
            ...
            case 12:    return lx.hsb(lxh() + NoiseUtils.noise(Pn.x,Pn.y,Pn.z + (NoiseMove+50000)/1000.f)*200,
                85,c1c(Pn.y < NoiseUtils.noise(Pn.x + NoiseMove/2000.f,Pn.z)*(1+amp)-amp/2.-.1 ? 1 : 0));    // noise

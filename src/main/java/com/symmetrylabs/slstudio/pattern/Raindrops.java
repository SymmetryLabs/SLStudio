...
public class Raindrops extends SLPattern<SLModel> {
    ...
    public final CompoundParameter sizeVar = new CompoundParameter("SizeVar", 0.5);
    public final CompoundParameter speedP = new CompoundParameter("Speed", -1000, -7000, -300);
    public final CompoundParameter speedVar = new CompoundParameter("SpeedVar", 0.5);
    ...
    class Raindrop {
        ...
        Raindrop() {
            this.radius = (float)((model.yRange*0.4f)*(size.getValuef() + random(-0.5f, 0.5f)));
        ...
        getVectorList().parallelStream().forEach(p -> {
            ...
            for (Raindrop raindrop : raindrops) {
                if (p.x >= (raindrop.p.x - raindrop.radius) && p.x <= (raindrop.p.x + raindrop.radius) &&
                        ...
                        p.z >= (raindrop.p.z - raindrop.radius) && p.z <= (raindrop.p.z + raindrop.radius)) {
                    float d = ((float)dist(raindrop.p.x, raindrop.p.y, raindrop.p.z, p.x, p.y, p.z)) / raindrop.radius;
                    if (d < 1) {

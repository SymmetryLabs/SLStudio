...
import com.symmetrylabs.slstudio.pattern.explosions.*;
...
public class CapExplosions extends SLPatternWithMarkers {
    ...
    private int hack_num_explosions;

    public CapExplosions(LX lx) {
        super(lx);
...
//                Iterator<L8onExplosion> iter = this.explosions.iterator();

//                while (iter.hasNext()) {
//                    L8onExplosion explosion_0 = iter.next();
//
//                    if (explosion_0 == explosion)
//                        iter.remove();
//                }
    ...
    private void initExplosions(LXPoint p) {
//        int num_explosions = (int) numExplosionsParameter.getValue();
        int num_explosions = hack_num_explosions;
...
//                LXPoint new_center_point = model.points[pointRandom.nextInt(model.points.length)];
    ...
    void advanceSimulation(double deltaSec) {

...
public class TreeWaves extends TreePattern {
    ...
    public void run(double deltaMs) {
        ...
        for (TreeModel.Leaf leaf : tree.getLeaves()) {
            ...
            int idx2 = Math.round((bins.length-1) * (len2 * (0.2f + leaf.point.xn))) % bins.length;
            int idx3 = Math.round((bins.length-1) * (len3 * (1.7f - leaf.point.xn))) % bins.length;

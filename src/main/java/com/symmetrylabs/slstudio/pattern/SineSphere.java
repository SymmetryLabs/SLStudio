...
import static com.symmetrylabs.util.MathConstants.TWO_PI;
...
public class SineSphere extends APat {
    ...
    private CompoundParameter yrotspeed = new CompoundParameter("yspeed", 3000, 1, 10000);
    private CompoundParameter yrot2speed = new CompoundParameter("y2speed", 4000, 1, 15000);
    ...
    private CompoundParameter speed = new CompoundParameter("nspeed", .00001, .01);
    ...
    public CompoundParameter huespread = new CompoundParameter("Hue", 0, 180);
    public CompoundParameter widthparameter= new CompoundParameter("Width", 20, 1, 60);
    public CompoundParameter vibration_magnitude = new CompoundParameter("Vmag", 20, 2, modelrad/2);
    public CompoundParameter scale = new CompoundParameter("Scale", 1, .1, 5);
    private int pitch = 0;
...
//    Pick Galaxy, STime;
    ...
    class Sphery {
        ...
        void setVelocity(PVector vel) {this.velocity=vel;}
        void setAcceleration(PVector acc) {this.accel= acc;}
        ...
        float distfromcirclecenter(PVector here)
        {
        ...
        void run(double deltaMs) { velocity.mult((float)deltaMs); this.center.add(velocity);  this.noiseTime=(float)deltaMs*speed.getValuef();}

        int spheryvalue (PVector p) {
        ...
        int ellipsevalue(float px, float py, float pz , float f1xc, float f1yc, float f1zc, float f2xc, float f2yc, float f2zc)
        {
//switch(sShpape.cur() ) {}
            return lx.hsb(huespread.getValuef()*5*px, dist(model.xMax-px, model.yMax-py, model.zMax-pz, f1xc, f1yc, f1zc) ,
                max(0, 100 - 100*widthparameter.getValuef() *
                    abs( (float)((dist(px, py, pz, f1xc, ybounce.getValuef(), f1zc) +
...
//        }
    ...
    int blendIfColor(int c1, int c2, int mode) {
        if (c2 != 0) {

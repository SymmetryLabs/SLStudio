...
public class HDWorms extends SLPattern<StripsModel<Strip>> {
    ...
    private LXVector randEdge() {
        return (LXUtils.random(0, 2) < 1) ?
            new LXVector(
                ((float) LXUtils.random(0, 2) < 1) ? model.xMin : model.xMax, randY(), Z_MID_LATE) :
    ...
    private void reset(dCursor c) {
        switch (animNum()) {
            ...
            case 3: // sideways
                ...
                float yLin = randX();
                c.setDest(lattice.getClosest(new LXVector(
                    bLeft ? 0 : model.xMax, yLin, Z_MID_LATE)).v, nConfusion);
    ...
    class dLattice {
        ...
        dVertex v0(Strip s) {
            return (dVertex) s.obj1;
        }

        dVertex v1(Strip s) {
            return (dVertex) s.obj2;
        }
        ...
        public dLattice(StripsModel<Strip> model) {
            ...
            for (Strip s1 : strips) {
                for (Strip s2 : strips) {
                    ...
                    if (dist2(s1, 0, s2, 0) < 20) {
                        c++;
                        addJoint(v1(s1), v0(s2));
                        addJoint(v1(s2), v0(s1));
                    }
                    if (dist2(s1, 0, s2, 27) < 20) {
                        c++;
                        addJoint(v1(s1), v1(s2));
                        addJoint(v0(s2), v0(s1));
                    }
                    if (dist2(s1, 27, s2, 0) < 20) {
                        c++;
                        addJoint(v0(s1), v0(s2));
                        addJoint(v1(s2), v1(s1));
                    }
                    if (dist2(s1, 27, s2, 27) < 20) {
                        c++;
                        addJoint(v0(s1), v1(s2));
                        addJoint(v0(s2), v1(s1));
                    }
                    if (c > 0) continue;

...
public class BlobFollower implements com.symmetrylabs.slstudio.util.MarkerSource {
    com.symmetrylabs.slstudio.util.BlobTracker tracker;
    ...
    public BlobFollower(com.symmetrylabs.slstudio.util.BlobTracker tracker) {
        this.tracker = tracker;
        this.followers = new CopyOnWriteArrayList<com.symmetrylabs.slstudio.util.BlobFollower.Follower>();
    }

    public void advance(float deltaSec) {
        List<com.symmetrylabs.slstudio.util.BlobTracker.Blob> blobs = tracker.getBlobs();

        for (com.symmetrylabs.slstudio.util.BlobTracker.Blob b : blobs) {
...
//        System.out.println("followers: " + followers.size());
    ...
    public class Follower {
        ...
        void advance(float deltaSec, com.symmetrylabs.slstudio.util.BlobTracker.Blob blob) {
            ageSec += deltaSec;
            if (blob == null) {
                if (value < fadedLevel) {
                    expired = true;
    ...
    public com.symmetrylabs.slstudio.util.BlobTracker.Blob findClosestBlob(
        PVector p,
        List<com.symmetrylabs.slstudio.util.BlobTracker.Blob> blobs,
        float range
    ) {
        com.symmetrylabs.slstudio.util.BlobTracker.Blob closest = null;
        float minDist = 0;
        for (com.symmetrylabs.slstudio.util.BlobTracker.Blob b : blobs) {
    ...
    public Collection<com.symmetrylabs.slstudio.util.Marker> getMarkers() {
        ...
        for (Follower f : followers) {
            markers.add(new com.symmetrylabs.slstudio.util.OctahedronWithArrow(f.pos, f.value * 24, 0x0000ff, f.vel, 0xffff00));

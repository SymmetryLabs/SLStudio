...
public class MFCC  {
    ...
    public float[][] process(float[] doubleInputBuffer) {
        final float[][] melSpec = dctMfcc(doubleInputBuffer);
        return melSpec;
    }
...
//        }
    ...
    private float[] getWindow(){
        ...
        for (int i = 0; i < n_fft; i++){
            win[i] = 0.5f - 0.5f * ((float) Math.cos(2.0f * Math.PI * i / n_fft));
        }
        return win;
    }
    ...
    private float[][] powerToDb(float[][] melS){
        ...
        for (int i = 0; i < melS.length; i++){
            for (int j = 0; j < melS[0].length; j++){
                ...
                if (magnitude > 1e-10){
                    log_spec[i][j]=10.0f* ((float) log10(magnitude));
                }else{
    ...
    private float[][] dctFilter(int n_filters, int n_input){
        ...
        for (int i = 0; i < n_input; i++){
            samples[i] = ((float) ((1f + 2f * i) * Math.PI / (2.0f * (n_input))));
        }
        for (int j = 0; j < n_input; j++){
            basis[0][j] = 1.0f/ ((float) Math.sqrt(n_input));
        }
    ...
    private float[] fftFreq() {
        ...
        for (int i = 0; i < 1+n_fft/2; i++){
            freqs[i] = (0 + ((float) ((sampleRate / 2) / (n_fft / 2))) * i);
        }
        return freqs;
    }
    ...
    private float[] melFreq(int numMels) {
        ...
        LowFFreq[0] = (float)fMin;
        HighFFreq[0] = (float)fMax;
    ...
    private float[] melToFreqS(float[] mels) {
        ...
        for (int i = 0; i < mels.length; i++) {
            freqs[i] = 700.0f * (((float) Math.pow(10, mels[i] / 2595.0f)) - 1.0f);
        }
        return freqs;
    }
    ...
    protected float[] freqToMelS(float[] freqs) {
        ...
        for (int i = 0; i < freqs.length; i++){
            mels[i] = 2595.0f * ((float) log10(1.0 + freqs[i] / 700.0f));
        }
        return mels;
    }
...
//    // hz to mel, Slaney, librosa
    protected float[] freqToMel(float[] freqs) {
        ...
        final float min_log_hz = 1000.0f;                         // beginning of log region (Hz)
        final float min_log_mel = (min_log_hz - f_min) / f_sp ;  // # same (Mels)
        final float logstep = ((float) Math.log(6.4)) / 27.0f;              // step size for log region

        for (int i = 0; i < freqs.length; i++) {
            if (freqs[i] < min_log_hz){
                mels[i] = (freqs[i] - f_min) / f_sp;
            }else{
                mels[i] = ((float) min_log_mel) + ((float) Math.log(freqs[i] / min_log_hz)) / ((float) logstep);
            }
        }
        return mels;
    }

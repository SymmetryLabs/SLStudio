package com.symmetrylabs.shows.cuddlefish.ui;
...
public class UICuddlefishModelingTool extends UI2dContainer {
    ...
    private void illuminateStrip(int globalIndex) {
        ...
        heronarts.lx.LXChannel ch = com.symmetrylabs.shows.cuddlefish.CuddlefishShow.illumChannel;
        ...
        if (ch == null) {
            ...
            com.symmetrylabs.shows.cuddlefish.CuddlefishShow.illumChannel = ch;
        }
        // Find StripIlluminator fresh from channel pattern list (static ref may be stale after reload)
        com.symmetrylabs.shows.cuddlefish.StripIlluminator illum = null;
        for (heronarts.lx.LXPattern p : ch.patterns) {
            if (p instanceof com.symmetrylabs.shows.cuddlefish.StripIlluminator) {
                illum = (com.symmetrylabs.shows.cuddlefish.StripIlluminator) p;
                break;
            }
        }
        ...
        if (illum == null) {
            ...
            ch.addPattern(illum);
            com.symmetrylabs.shows.cuddlefish.CuddlefishShow.illuminator = illum;
        }
    ...
    public UICuddlefishModelingTool(UI ui, float x, float y, float w) {
    ...
    private void buildStripGrid(float[][] existingStrips, int insertAfterGlobalIndex, int insertInUniverse) {
        ...
        System.out.println("UICuddlefishModelingTool: built grid with " + globalRow + " total strips across " + UNIVERSE_COUNT + " universes");
    }
    ...
    public void saveMappingToDisk() {
        ...
        try (FileWriter writer = new FileWriter(f)) {
            ...
            System.out.println("UICuddlefishModelingTool: saved " + file.strips.length + " strips to " + MAPPING_FILE);
        } catch (IOException e) {
            System.err.println("UICuddlefishModelingTool: failed to save mapping");
            e.printStackTrace();
        }
    }
    ...
    public static MikeyMappingFile loadFileFromDisk() {
        File f = new File(MAPPING_FILE);
        if (!f.exists()) {
            System.out.println("UICuddlefishModelingTool: no mapping file found, using defaults");
            return null;
        }
        try (FileReader reader = new FileReader(f)) {
            ...
            if (file == null || file.stripCounts == null || file.strips == null) {
                System.out.println("UICuddlefishModelingTool: mapping file empty/invalid, using defaults");
                return null;
            }
            System.out.println("UICuddlefishModelingTool: loaded " + file.strips.length + " strips across " + file.stripCounts.length + " universes");
            return file;
        } catch (Exception e) {
            System.err.println("UICuddlefishModelingTool: failed to load mapping, using defaults");
            e.printStackTrace();
            return null;
        }
    }
     ...
     * Called by CuddlefishShow at model-build time.
     */

...
public class UniverseSelectorMikey extends SLPattern<StripsModel> {
    ...
    public UniverseSelectorMikey(LX lx) {
        super(lx);
        ...
        for (int u = 0; u < UNIVERSE_COUNT; u++) {
            ...
            if (u >= 20 && u <= 23) {
                System.out.println("UniverseSelectorMikey: U" + (u + 1) + " -> strips[" + stripStart + ".." + (stripIndex - 1) + "] count=" + counts[u] + " pixels=" + pts.size());
            }
        }
        System.out.println("UniverseSelectorMikey: total strips=" + strips.size() + " total universes=" + UNIVERSE_COUNT);

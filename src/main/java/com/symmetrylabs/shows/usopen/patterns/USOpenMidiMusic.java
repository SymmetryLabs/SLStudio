...
public class USOpenMidiMusic extends SLPattern<CubesModel> {
    ...
    private class Sweep extends LXLayer {
        ...
        Sweep() {
            super(USOpenMidiMusic.this.lx, USOpenMidiMusic.this);
        ...
        public void run(double deltaMs) {
            ...
            for (LXVector p : getVectors()) {
                blendColor(p.index, lx.hsb(
                    hue.getValuef() + 0.2f*Math.abs(p.x - model.cx) + 0.2f*Math.abs(p.y - model.cy),
                    sat.getValuef(),

...
public class USOpenSwim extends LXPattern {
    ...
    public void run(double deltaMs, PolyBuffer.Space space) {
        ...
        StreamSupport.stream(
            ...
            float hue_color = hue.getValuef() + hueScale.getValuef()
                * ((float) Math.abs(p.x - model.xMax / 2.) * .01f
                + (float) Math.abs(p.y - model.yMax / 2) * .6f
                + (float) Math.abs(p.z - model.zMax));

            colors[p.index] = LXColor.hsb(hue_color, sat.getValuef(), v1);
        });
        markModified(SRGB8);
    }
}

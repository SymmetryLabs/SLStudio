...
public class USOpenNoise extends DPat {
    ...
    public final CompoundParameter hueVariance = new CompoundParameter("HueVar", 0.4, 0, 1);
    ...
    public int CalcPoint(PVector p) {
        ...
        if (currModeIndex == 6 || currModeIndex == 7) {
            ...
            return lx.hsb(hue.getValuef(), sat.getValuef(), 100 * bri);
        }
        ...
        for (int i = 0; i < n.length; i++)
            if (n[i].Active()) {
                ...
                col = PImage.blendColor(col, lx.hsb(hue.getValuef() + (nDat.hue*hueVariance.getValuef()), sat.getValuef(), c1c(bri)), ADD);
            }

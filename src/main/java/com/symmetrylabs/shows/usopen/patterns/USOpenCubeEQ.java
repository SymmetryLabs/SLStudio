...
public class USOpenCubeEQ extends SLPattern<SLModel> {
    ...
    public void run(double deltaMs, PolyBuffer.Space space) {
        ...
        model.forEachPoint((start, end) -> {
            for (LXVector v : getVectors(start, end)) {
                ...
                float h = 480 + hue.getValuef() - min(clrConst * v.y, 120);
                float b = constrain(edgeConst * (value * model.yMax - v.y), 0, 100);
                colors[v.index] = LXColor.hsb(h, sat.getValuef(), b);
            }
        });
        markModified(SRGB8);
    }

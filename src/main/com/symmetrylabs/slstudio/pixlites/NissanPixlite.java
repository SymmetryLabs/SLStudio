...
public class NissanPixlite extends LXOutputGroup {
    ...
    public NissanPixlite(LX lx, String ipAddress) {
        super(lx);
        this.lx = lx;
        this.ipAddress = ipAddress;
    }

    public NissanPixlite addPixliteOutput(PointsGrouping pointsGrouping) {
        try {
            addChild(new NissanPixliteOutput(pointsGrouping));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return this;
    ...
    private class NissanPixliteOutput extends LXDatagramOutput {
        private final int MAX_NUM_POINTS_PER_UNIVERSE = 170;
        private final int outputIndex;
        private final int firstUniverseOnOutput;

        private NissanPixliteOutput(PointsGrouping pointsGrouping) throws SocketException {
            super(lx);

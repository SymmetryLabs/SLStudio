...
public SLModel buildModel() {
  ...
  /* Ola Strips ------------------------------------------------------*/
  List<OlaStrip> olaStrips = new ArrayList<OlaStrip>();

  for (StripConfig stripConfig : OLA_STRIP_CONFIG) {
    Strip.Metrics metrics = new Strip.Metrics(stripConfig.numPoints, stripConfig.spacing);
    String id = ((StripConfig)stripConfig).id;
    ...
    OlaStrip olaStrip = new OlaStrip(id, metrics, stripConfig.yRot, globalTransform, true);
    olaStrips.add(olaStrip);
    strips.add((Strip)olaStrip);
  ...
  return new SLModel(towers, allCubesArr, strips, olaStrips);
}

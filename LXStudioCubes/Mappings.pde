...
public CubesModel buildModelFromJson(LXTransform globalTransform) {
  String jsonStr;
  try {
    ...
    List<CubesModel.Tower> emptyTowers = new ArrayList<CubesModel.Tower>();
    CubesModel.Cube emptyCubes[] = new CubesModel.Cube[0];
    List<CubesModel.Strip> emptyStrips = new ArrayList<CubesModel.Strip>();
    return new CubesModel(emptyTowers, emptyCubes, emptyStrips);
  }

  JsonArray json = new Gson().fromJson(jsonStr, JsonArray.class);

  List<CubesModel.Tower> towers = new ArrayList<CubesModel.Tower>();
  CubesModel.Cube[] cubes = new CubesModel.Cube[200];
  int cubeIndex = 0;
  ...
  for (JsonElement element : json) {
    ...
    List<CubesModel.Cube> tower = new ArrayList<CubesModel.Cube>();
    ...
    CubesModel.Cube cube = (CubesModel.Cube)automappingController.getModelForId(macAddress, transform);
    ...
    towers.add(new CubesModel.Tower(new Integer(cubeIndex).toString(), tower));
  }
  List<CubesModel.Strip> emptyStrips = new ArrayList<CubesModel.Strip>();
  return new CubesModel(towers, cubes, emptyStrips);
}

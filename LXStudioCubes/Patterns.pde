...
public class Snap extends LXPattern {
  ArrayList<LXVector> particles;
  ArrayList<LXVector> destinations;
  ArrayList<Float> spreads;
  ...
  public Snap(LX lx) {
    ...
    particles = new ArrayList<LXVector>();
    destinations = new ArrayList<LXVector>();
    spreads = new ArrayList<Float>();
  ...
  LXPoint randomPoint() {
    int i = int(random(model.points.length));
    return model.points[i];
  }

  float nrand(float max) {
    float v = random(0, 2) - 1;
    return v * max;
  }
...
public static abstract class SLPattern extends LXPattern {
  ...
  public SLPattern(LX lx) {
    ...
    if (lx.model instanceof SLModel) {
      ...
      this.model = new SLModel(new ArrayList<Tower>(), new Cube[0], new ArrayList<Strip>());
    }
  }
}

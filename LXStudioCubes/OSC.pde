...
class EssentiaOSCListener implements LXOscListener {
  private LXOscEngine.Transmitter transmitter = null;
  private final String BASE_ROUTE = "/lx/modulation/Audio Analyzation";

  public EssentiaOSCListener(LX lx) {
    try {
        InetAddress address = InetAddress.getByName("localhost");
        int port = lx.engine.osc.DEFAULT_RECEIVE_PORT;

        this.transmitter = lx.engine.osc.transmitter(address, 3030);
    } catch (UnknownHostException e) {
        println(e);
    } catch (SocketException e) {
        println("Could not create osc transmitter: " + e.getMessage());
    }
  }
  public void oscMessage(OscMessage message) {
    ...
    try {
      ...
      println("Could not transmit message" + e.getMessage());
    }
  }
}
...
//               channel.tx = model.cx + rx * model.xRange/2;
//               channel.ty = model.cy + rz * model.yRange/2;
//               channel.tz = model.cz + ry * model.zRange/2;
//             }
//           } else {

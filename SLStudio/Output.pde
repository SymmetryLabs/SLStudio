...
class SLController extends LXOutput {
  ...
  void onSend(int[] colors) {
    ...
    Branch branch = null;
    if ((outputControl.testBroadcast.isOn() || isBroadcast) && model.branches.size() > 0) {
      branch = model.branches.get(0);
    } else {
      for (Branch b : model.branches) {
    ...
    if (branch != null) {
      ...
      for (Strip s : branch.strips) {
        setPixel(counter++, colors[s.points[0].index]);
        setPixel(counter++, colors[s.points[1].index]);
        setPixel(counter++, colors[s.points[2].index]);
        setPixel(counter++, colors[s.points[3].index]);
        setPixel(counter++, colors[s.points[2].index]);
        setPixel(counter++, colors[s.points[1].index]);
        setPixel(counter++, colors[s.points[0].index]);
      }

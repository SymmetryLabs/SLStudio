...
public class FlockWave extends ThreadedPattern {
    ...
    private DiscreteParameter palette = new DiscreteParameter("palette", SkyPaletteLibrary.getInstance().getNames());  // selected colour palette
    ...
    private List<Bird> removeExpiredBirds() {
        List<Bird> expired = new ArrayList<>();
        for (Bird b : birds) {
            if (b.hasExpired) {
                expired.add(b);
            }
    ...
    public void render(double deltaMs, List<LXPoint> points, IntBuffer pointColors) {
        SkyPaletteLibrary.ColorPalette pal = SkyPaletteLibrary.getInstance().getPalette(palette.getOption());
        ...
        for (int i = 0; i < points.size(); ++i) {
            ...
            int color = pal.getColor(FastMath.signum(val) * FastMath.pow(FastMath.abs(val), bias) + palShift.getValuef());
            pointColors.put(i, color);
        }
    }

...
public class Balance extends LXPattern {
    ...
    public void run(double deltaMs) {
        ...
        for (LXVector p : projection) {
            ...
            float hue_color = palette.getHuef() + hueScale.getValuef() * ((float)Math.abs(p.x - model.xMax / 2f) + (float)Math.abs(p.y - model.yMax / 2f) * .2f + (float)Math.abs(p.z - model.zMax/2f) * .5f);
            ...
            for (Sphere s : spheres) {
                ...
                float d = (float)Math.sqrt((float)Math.pow(x_dist, 2) + (float)Math.pow(p.y - s.y, 2) + (float)Math.pow(p.z - sphere_z, 2));
